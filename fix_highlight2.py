...
old_ref = r'const applyHighlightRef = useRef<\(\(selNode: GraphNode \| null, selFolder: string \| null, mode: "connection" \| "folder"\) => void\) \| null>\(null\);'
new_ref = 'const applyHighlightRef = useRef<((selNode: GraphNode | null, selFolder: string | null, mode: "connection" | "folder", reportNodes?: Map<string, any>) => void) | null>(null);'
content = re.sub(old_ref, new_ref, content)

# 2. Update useEffect for applyHighlightRef
content = re.sub(
    r'applyHighlightRef\.current\(activeSelectedNode, activeSelectedFolder, highlightMode\);\s*\}\s*\}, \[highlightMode, activeSelectedNode, activeSelectedFolder\]\);',
    r'applyHighlightRef.current(activeSelectedNode, activeSelectedFolder, highlightMode, reportSelectedNodes);\n    }\n  }, [highlightMode, activeSelectedNode, activeSelectedFolder, reportSelectedNodes]);',
    content
)

# 3. Apply highlighting logic
# Find the start and end of applyHighlight
start_idx = content.find('const applyHighlight = (selNode: GraphNode | null, selFolder: string | null, mode: "connection" | "folder") => {')
if start_idx != -1:
...
with open("src/components/KnowledgeGraphModal.tsx", "w", encoding="utf-8") as f:
    f.write(content)
print("Updated highlighting properly!")
