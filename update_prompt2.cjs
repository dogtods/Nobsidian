...
3. テキストが枠からはみ出さないよう、以下を徹底すること。
- flowchartのノード内テキストは1行あたり全角12文字程度を目安に、長い場合は<br>で改行する
- timelineの各項目テキストも同様に短く区切り、必要に応じて<br>で改行する
- ノードIDと表示テキストを分離し、表示テキストのみを簡潔にする

4. 積み上げ棒グラフなど複数系列を色で区別する場面では、色の濃淡ではなく系列名を凡例(legend)やラベルとして明示し、白黒でも判別できるようにすること。Mermaidのxychart-betaはハッチング柄に対応していないため、系列ごとに棒グラフを分けて並べる、または系列名を直接ラベル表示するなどの代替手段で対応すること。
