...
old_logic = """    const tSheetName = (targetSheetName && String(targetSheetName).trim()) ? String(targetSheetName).trim() : "Notes";
    let targetSheet = targetSs.getSheetByName(tSheetName);
    if (!targetSheet) targetSheet = targetSs.insertSheet(tSheetName);"""

new_logic = """    let tSheetName = (targetSheetName && String(targetSheetName).trim()) ? String(targetSheetName).trim() : "Notes";
    let targetSheet = targetSs.getSheetByName(tSheetName);
...
if old_logic in content:
    ...
    with open("src/gasScriptCode.ts", "w") as f:
        f.write(content)
    print("Successfully updated GAS logic.")
else:
