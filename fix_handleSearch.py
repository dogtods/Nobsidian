...
end_idx = content.find("  // External Drag Handlers for AI Nodes")
...
if start_idx != -1 and end_idx != -1:
    ...
    if (firstFound && svgRef.current && zoomRef.current && containerRef.current) {
      ...
      d3.select(svgRef.current)
        .transition()
        .duration(650)
        .call(zoomRef.current.transform, d3.zoomIdentity.translate(width / 2 - (firstFound.x || 0), height / 2 - (firstFound.y || 0)));
    }
  };
